...
class Meteo_DataService(QWidget):
    def __init__(self):
        ...
        self.setWindowTitle('镇江气象资料证明应用软件 V1.0')
        self.resize(1117,615)
        self.initUI()
    def initUI(self):
        ...
        self.MinValue_LineEdit.setValidator(QDoubleValidator())
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.Fixed, QtWidgets.QSizePolicy.Fixed)
        sizePolicy.setHorizontalStretch(0)
        sizePolicy.setVerticalStretch(0)
        ...
        self.Runpage_Button.setCheckable(True)
        ...
        self.Usage_Label.setText("用途：")
        self.Usage_Combox.setItemText(0, "高温补贴")
        self.Usage_Combox.setItemText(1, "保险理赔")
        ...
        self.Condition_Label.setText("参数值：")
        self.Condition_Combox.setItemText(0, "时段极值")
        self.Condition_Combox.setItemText(1, "时段累计值")
        self.Condition_Combox.setItemText(2, "时段均值")
        self.Condition_Combox.setItemText(3, "日值")
        ...
        self.Runpage_Button.setText("启动网页")
        self.Generate_Button.setText("生成文件")
        self.Preview_Button.setText("预览文件")
    ...
    def Preview(self):
        ...
        # word.Documents.Open(r'C:\Users\DC\Desktop\2021年气象综述.docx')
        pass
    ...
    def Generate(self):
        try:
            ...
            print(e)
        self.startdatetime = self.Starttime_DateEdit.dateTime().toString("yyyy-MM-dd HH:mm:ss")
        self.enddatetime = self.Endtime_DateEdit.dateTime().toString("yyyy-MM-dd HH:mm:ss")
        ...
        if '时' in self.selectedCon:
            self.dataNeed = self.hourdata[self.selectedInfo]
        else:
            self.dataNeed= self.daydata[self.selectedInfo]
        print(self.dataNeed)
    ...
    def Runpage(self):
        ...
        stationSelect = Select(self.browser.find_element_by_name("tt"))
        stationSelect.select_by_visible_text(self.station + '站')
        sleep(1)
        self.Startdate = self.Starttime_DateEdit.date().toString("yyyy-MM-dd")
        ...
        for idate in dayindex:
            ...
            # 查询
            data_content = self.browser.find_element_by_xpath(
                '//*[@id="Layer1"]/table/tbody').find_elements_by_tag_name('td')
            datalist = []
            for data1 in data_content:
                datalist.append(data1.text)
            if self.station in ['句容', '扬中', '丹阳']:
                datalist = np.array(datalist[58:-14]).reshape(25, -1)
                day_array = np.r_[day_array, datalist[-1, [0, 1, 2, 4, 21, 44]].reshape(-1, 6)]
                hour_array = np.r_[hour_array, datalist[:-1, [0, 1, 2, 4, 21, 44]].reshape(-1, 6)]
            elif self.station == '镇江':
                datalist = np.array(datalist[63:-18]).reshape(25, -1)
                day_array = np.r_[day_array, (datalist[-1, [0, 1, 2, 4, 21, 48]]).reshape(-1, 6)]
                hour_array = np.r_[hour_array, datalist[:-1, [0, 1, 2, 4, 21, 48]].reshape(-1, 6)]
            else:
                datalist = np.array(datalist[30:-7]).reshape(25, -1)
                day_array = np.r_[day_array, datalist[-1, [0, 1, 2, 4, 21, 22]].reshape(-1, 6)]
                hour_array = np.r_[hour_array, datalist[:-1, [0, 1, 2, 4, 21, 22]].reshape(-1, 6)]

        self.hourdata = pd.DataFrame(hour_array, columns=['降水', '整点气温', '最高气温', '最低气温', '极大风', '能见度'], index=hourindex)
        self.daydata = pd.DataFrame(day_array, columns=['降水', '整点气温', '最高气温', '最低气温', '极大风', '能见度'],index=dayindex)
        self.hourdata.to_pickle('hourdata_pickle')
        self.daydata.to_pickle('daydata_pickle')
